...
std::vector<Point2f> color_keypoints(4);  
		  try
		  {
				...
				col_3D[3] = Point3f(0.24,0.245,0); //For the red dots in the box
				/*col_3D[0] = Point3f(-0.1875,-0.12,0); 
				col_3D[1] = Point3f(0.1875,-0.12,0);
				col_3D[2] = Point3f(-0.1875,0.12,0); 
				col_3D[3] = Point3f(0.1875,0.12,0);*///For the closet dots
				//Thresholding
				...
				if(keyPointsBlob.size()==4)
				{
					...
					measc.at<float>(0) = puntoc.x;//Uncomment for color
	  ...
	  //-- Draw lines between the corners (the mapped object in the scene - image_2 )
				...
				if(estimatedc.at<float>(0)!=estimatedc.at<float>(0))
				{
					
					estimatedc.at<float>(0)=estimatedcx;
					estimatedc.at<float>(1)=estimatedcy;
					estimatedc.at<float>(2)=estimatedcz;
					estimatedc.at<float>(3)=estimatedcvx;
					estimatedc.at<float>(4)=estimatedcvy;
					estimatedc.at<float>(5)=estimatedcvz;
					printf("hola");
				}
				estimatedcx=estimatedc.at<float>(0);
				estimatedcy=estimatedc.at<float>(1);
				estimatedcz=estimatedc.at<float>(2);
				estimatedcvx=estimatedc.at<float>(3);
