...
float estimatedvz;
	...
	void imageCallback(const sensor_msgs::ImageConstPtr& msg)
	{
		...
		ofstream myfile("/home/edrone/posicion_feature3.txt",ios_base::app);
		int count=0;
...
std::vector<Point2f> color_keypoints(4);  
		  try
		  {
				...
				col_3D[2] = Point3f(-0.24,0.24,0); 
				col_3D[3] = Point3f(0.24,0.24,0); //For the red dots in the box
				...
				if(keyPointsBlob.size()==4)
				{
					...
					//meas.at<float>(0) = punto.x;//Uncomment for color
	  ...
	  //-- Draw lines between the corners (the mapped object in the scene - image_2 )
				...
				myfile <<pospt.x<<"	"<<fx<<"	"<<pospt.y<<"	"<<fy<<"	"<<pospt.z<<"	"<<fz<<"\n";
