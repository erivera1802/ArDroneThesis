...
float Kplace(float Px,float Velx,float delay)
{
	float K1=0.4528;
	float K2=0.1733;
	float K3=0.9774;
	float Uc=0.35-K1*Px-K2*Velx-K3*delay;
	return Uc;
...
{
	ofstream myfile("/home/edrone/PDStep.txt",ios_base::app);
  ros::NodeHandle neu;
	...
	actual=ros::Time::now().toSec()-start_time;
		...
		if( actual> takeoff_time && actual< takeoff_time+fly_time)
		{
			...
			{
				reference.at<float>(1)=0.3;
				printf("Step! \n");
			}
			
			kontrol=PID(estado,reference);
			Uy=Kplace(estado.at<float>(1),estado.at<float>(4),estimated.at<float>(1));	
			...
			myfile << twist_msg.linear.x<<"	"<<estado.at<float>(0)<<"	"<<reference.at<float>(0)<<"	"<<ex<<"	"<< twist_msg.linear.y<<"	"<<estado.at<float>(1)<<"	"<<reference.at<float>(1)<<"	"<<ey<<"	"<<estimated.at<float>(1)<<"	"<<tiempo<<"\n";
			//myfile << twist_msg.linear.y<<"	"<<estado.at<float>(1)<<"	"<<reference.at<float>(1)<<"	"<<ey<<"	"<<tiempo<<"\n";
			//myfile << twist_msg.linear.x<<"	"<<estado.at<float>(3)<<"	"<<tiempo<<"\n";
...
//Transition Matrix A
	...
	kf.transitionMatrix.at<float>(0,0)=0.9317;
	kf.transitionMatrix.at<float>(1,0)=0.0;
	kf.transitionMatrix.at<float>(0,1)=1.793;
	kf.transitionMatrix.at<float>(1,1)=0.7384;
