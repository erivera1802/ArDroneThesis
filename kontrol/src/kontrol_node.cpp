...
float Kplace(float Px,float Velx,float delay)
{
	...
	float K3=6.3071;
	float N=4.0096;
	float Uc=15-K1*Px-K2*Velx-K3*delay;
	return Uc;
...
{
	ofstream myfile("/home/edrone/KpObj.txt",ios_base::app);
  ros::NodeHandle neu;
	...
	actual=ros::Time::now().toSec()-start_time;
		...
		if( actual> takeoff_time && actual< takeoff_time+fly_time)
		{
			...
			Uy=Kplace(estimated.at<float>(0),estimated.at<float>(1),estimated.at<float>(2));	
